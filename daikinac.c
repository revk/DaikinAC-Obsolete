...
#undef	c
  ...
  int info = 0, modeoff = 0, modeon = 0, modeauto = 0, modeheat = 0, modecool = 0, modedry = 0, modefan = 0, dolock = 0;
...
#undef c
    ...
    if (!poptPeekArg (optCon) || modeauto + modeheat + modecool + modedry + modefan + (setmode ? 1 : 0) > 1 || modeoff + modeon + (setpow ? 1 : 0) > 1)
      {
	...
	while ((ip = poptGetArg (optCon)))
	  {
	    ...
	    size_t atemplen = 0, htemplen = 0, otemplen = 0, heatlen = 0, coollen = 0, dt1len = 0;
	    char *atempbuf = NULL, *htempbuf = NULL, *otempbuf = NULL, *heatbuf = NULL, *coolbuf = NULL, *dt1buf = NULL;
	    char atempm = 'M', htempm = 'M', otempm = 'M', dt1m = 'M';
		...
		{
		  xml_addf (svg, "+path@stroke=grey@fill=none@opacity=0.5@stroke-dasharray=1@stroke-width=0.5@d", "M%d 0v%d", x, svgheight);
		...
		{
		  xml_addf (svg, "+path@stroke=grey@fill=none@opacity=0.5@stroke-dasharray=1@stroke-width=0.5@d", "M0 %dh%d", svgheight - y, svgwidth);
	...
	{			// Auto temp
	  ...
	  if (newmode == 4 && offset <= -flip)
		...
		offset = maxoffset;
	      ...
	      terr += newtemp - rtemp;
	      if (sqldebug)
		warnx ("Set target %.1lf (%.1lf err %.1lf) atemp %.1lf ave: atemp %.1lf#%d stemp %.1lf#%d offset %.1lf", rtemp, newtemp, terr, atemp, atempq.sum / atempq.num, atempq.num, stempq.sum / stempq.num, stempq.num, offset);
	    }
	  if (newtemp > maxtemp)
	    newtemp = maxtemp;
	  if (newtemp < mintemp)
...
#undef c
	...
	if (db)
	  {			// Load temp history from database
	    ...
	    char lastfrate = 0;
	    SQL_RES *res = sql_safe_query_store_free (&sql,
						      sql_printf ("SELECT * FROM `%#S` WHERE `ip`=%#s AND `Updated`>=%#T ORDER BY `Updated`",
		...
		if (getstatus ())
		  {
		    ...
		    void check (char *tag, char *val)
		    {
		      ...
		      if (!strncmp (tag, "b_", 2) || (strncmp (tag, "f_", 2) && !strstr (tag, "pow") && !strstr (tag, "temp") && strcmp (tag, "mode") && !strstr (tag, "hum") && strcmp (tag, "adv")))
			return;
