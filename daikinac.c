...
#undef	c
  ...
  char *atemp = NULL;
  int info = 0, hotcold = 0, modeoff = 0, modeon = 0, modeauto = 0, modeheat = 0, modecool = 0, modedry = 0, modefan = 0;
	...
	while ((ip = poptGetArg (optCon)))
	  {
	    ...
	    SQL_RES *res = sql_safe_query_store_free (&sql,
						      sql_printf ("SELECT * FROM `%#S` WHERE `Updated` LIKE '%#S%%' AND `IP`=%#s", table,
								  svgdate, ip));
